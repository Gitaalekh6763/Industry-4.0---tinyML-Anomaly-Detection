...
const int MAX_MEASUREMENTS = 60;   // Start with a moderate number of measurements
const float MAD_SCALE = 1.4826;     // Scale MAD for consistency with SciPy
const float THRESHOLD = 30.0;        // Adjust based on the normal range of Mahalanobis distances
const int WAIT_TIME = 1000;         // 1 second delay between sample sets
const int SAMPLE_RATE = 200;         // Start with 50 Hz sample rate
...
void loop() {
  ...
  while (i<MAX_MEASUREMENTS) {
    if (millis() >= timestamp + (1000 / SAMPLE_RATE)) {
      ...
      sample[i][0] = msa.x_g;
      // Serial.println(sample[i][0]);
      sample[i][1] = msa.y_g;
      // Serial.println(sample[i][1]);
      sample[i][2] = msa.z_g;
      // Serial.println(sample[i][2]);
